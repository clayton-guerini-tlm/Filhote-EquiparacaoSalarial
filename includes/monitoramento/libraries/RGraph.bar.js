...
{
this.id = id;this.canvas = document.getElementById(id);this.context = this.canvas.getContext ? this.canvas.getContext("2d") : null;this.canvas.__object__ = this;this.type = 'bar';this.max = 0;this.stackedOrGrouped = false;this.isRGraph = true;this.uid = RGraph.CreateUID();RGraph.OldBrowserCompat(this.context);this.properties = {
...
this.data = data;this.coords = [];this.data_arr = RGraph.array_linearize(this.data);this.getShape = this.getBar;RGraph.Register(this);}
...
{
RGraph.FireCustomEvent(this, 'onbeforedraw');if(typeof(this.Get('chart.background.image')) == 'string'){
RGraph.DrawBackgroundImage(this);}
this.gutterLeft = this.Get('chart.gutter.left');this.gutterRight = this.Get('chart.gutter.right');this.gutterTop = this.Get('chart.gutter.top');this.gutterBottom = this.Get('chart.gutter.bottom');for (var i=0; i<this.data.length; ++i){
if(this.data[i] == null){
this.data[i] = 0;}
}
...
this.coords = [];this.max = 0;this.grapharea = RGraph.GetHeight(this) - this.gutterTop - this.gutterBottom;this.halfgrapharea = this.grapharea / 2;this.halfTextHeight = this.Get('chart.text.size') / 2;RGraph.background.Draw(this);RGraph.InstallUserClickListener(this, this.Get('chart.events.click'));RGraph.InstallUserMousemoveListener(this, this.Get('chart.events.mousemove'));RGraph.AllowTooltips(this);if(this.Get('chart.variant') == 'sketch'){
...
if(this.Get('chart.noxaxis') == false){
xTickGap = (this.canvas.width - this.gutterLeft - this.gutterRight) / this.data.length;if(typeof(this.Get('chart.numxticks')) == 'number'){
...
{
var canvas = e.target;var mouseCoords = RGraph.getMouseXY(e);obj = arguments[1] ? arguments[1] : this;for (var i=0; i<obj.coords.length; i++){
...
'object': obj, 'x': left, 'y': top, 'width': width, 'height': height, 'index': i
};}
}
return null;}
...
value = this.max - value;value = Math.abs(value) * -1;} else {
var value = ((this.grapharea - (mouseY - this.Get('chart.gutter.top'))) / this.grapharea) * (this.max - this.min)
value += this.min;}
return value;}
RGraph.Bar.prototype.getYCoord = function (value)
{
var yCoord;yCoord = ((value - this.min) / (this.max - this.min)) * this.grapharea;yCoord = this.canvas.height - this.gutterBottom - yCoord;return yCoord;}
